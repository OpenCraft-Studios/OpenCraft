...
public class GuiIngame extends GuiElement {
	...
	public void renderGameOverlay(final float float1, final boolean boolean2, final int integer3, final int integer4) {
		...
		if (oc.options.showDebugInfo.get()) {
			font.drawShadow(TITLE + " (" + this.mc.fpsString + ")", 2, 2, 0xFFFFFF);
			font.drawShadow(this.mc.debugInfoRenders(), 2, 12, 0xFFFFFF);
			font.drawShadow(this.mc.entityRenderingInfo(), 2, 22, 0xFFFFFF);
			font.drawShadow(this.mc.debugInfoEntities(), 2, 32, 0xFFFFFF);
			// display current coordinates and orientation
			font.drawShadow("X: " + this.mc.player.x + " Y: " + this.mc.player.y + " Z: " + this.mc.player.z, 2, 42,
					0xFFFFFF);
			font.drawShadow("Yaw: " + this.mc.player.yRot + " Pitch: " + this.mc.player.xRot, 2, 52, 0xFFFFFF);
	...
	private void renderVignette(float float1, final int integer2, final int integer3) {
		...
		t.beginQuads();
		{
			t.vertexUV(0.0, integer3, -90.0, 0.0, 1.0);
			t.vertexUV(integer2, integer3, -90.0, 1.0, 1.0);
			t.vertexUV(integer2, 0.0, -90.0, 1.0, 0.0);
