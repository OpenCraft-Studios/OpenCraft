...
public class PlayerController {
	...
	public boolean sendBlockRemoved(final int xCoord, final int yCoord, final int zCoord) {
		...
		if(block != null && setBlockWithNotify) {
			oc.sndManager.playSound(block.stepSound.stepSoundDir(), xCoord + 0.5f, yCoord + 0.5f, zCoord + 0.5f, (block.stepSound.soundVolume() + 1.0f) / 2.0f, block.stepSound.soundPitch() * 0.8f);
