		...
		for ( int i = integer1 - n; i <= integer1 + n; ++i ) {
			for ( int j = integer2 - n; j <= integer2 + n; ++j ) {
				...
				for ( int k = 0; k < nextInt; ++k ) {
					...
					for ( int l = 0; l < n7; ++l ) {
						this.a(integer1, integer2, arr, n4, n5, n6, this.rand.nextFloat() * 2.0f + this.rand.nextFloat(), this.rand.nextFloat() * PI_TIMES_2_f, (this.rand.nextFloat() - 0.5f) * 2.0f / 8.0f, 0, 0, 1.0);
					}
				}
			}
		}
