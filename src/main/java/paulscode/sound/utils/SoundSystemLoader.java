			...
			if (command != null && !command.substring(0, 1).equals("/")) {
				...
				if (command.equals("ADDLIBRARY")) {
					...
					if (sourcename.equals("")) {
						...
						s.CommandQueue(new paulscode.sound.CommandObject(paulscode.sound.CommandObject.NEW_SOURCE, priority, toStream, toLoop, sourcename, new paulscode.sound.FilenameURL(filename), x, y, z, attModel, distOrRoll));
						if (verbose)
